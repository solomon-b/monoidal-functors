...
## Upcoming
* Adds rank-2 `Traversable` classes for higher-kinded data: `Data.Functor.Rank2.Traversable`, `Data.Bifunctor.Rank2.Traversable`, and `Data.Trifunctor.Rank2.Traversable`. Their `sequence` distributes the interpretation functor out of a record over any functor / profunctor / trifunctor that is `Monoidal` with respect to tupling. Each ships a `GHC.Generics` default covering records of `f a` / `p a b` / `p a b c` fields. `Data.Functor.Rank2.Traversable` is polymorphic over the variance of the interpretation, generalizing `barbies`' `TraversableB`. At a covariant functor `sequence` is the `Applicative` collect, and at a contravariant functor it is the `Divisible` fold of a record of consumers into a single consumer of the record. The trifunctor version splits each field into its `First`, `Second`, and `Third` projections.
