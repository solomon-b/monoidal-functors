...
## Upcoming
* Adds `Data.Functor.Rank2.Traversable`, `Data.Bifunctor.Rank2.Traversable`, and `Data.Trifunctor.Rank2.Traversable` — rank-2 `Traversable` classes for higher-kinded data whose `sequence` distributes the interpretation functor out of a record over any functor / profunctor / trifunctor that is `Monoidal` with respect to tupling (the monoidal presentation of `Applicative`). Each ships a `GHC.Generics` default covering records of `f a` / `p a b` / `p a b c` fields. The functor version is the analogue of `barbies`' `TraversableB`; the trifunctor version splits each field into its `First`, `Second`, and `Third` projections.
